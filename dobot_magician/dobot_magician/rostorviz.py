...
class RosToRviz(Node):
    def __init__(self):
        ...
        self.get_logger().info('RosToRviz node started – /dobot_joint_states → /joint_states_robot')
    ...
    def listener_callback(self, msg):
        ...
        offset = math.pi / 2 
        q_servo = float(-(q2 + q3)) + offset

        new_msg = JointState()
        new_msg.header.stamp = self.get_clock().now().to_msg()
        new_msg.header.frame_id = ''

        # Nazwy muszą dokładnie odpowiadać jointom w pliku URDF
        new_msg.name = [
            'box_to_base',
            'base_to_reararm',
            'reararm_to_forearm',
        ...
        new_msg.position = [
            float(q1),                        # box_to_base     – obrót podstawy
            float(q2), 
            float(q3),                       # base_to_reararm – ramię tylne 
            float(q3 - q2) + math.pi / 2,          # reararm_to_forearm – ramię przednie
            float(-q3) + math.pi / 2,               # forearm_to_servo – kompensacja pionu
